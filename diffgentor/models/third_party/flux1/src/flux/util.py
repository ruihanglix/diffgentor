...
def load_flow_model(name: str, device: str | torch.device = "cuda", verbose: bool = True, local_path: str | None = None) -> Flux:
    # Loading Flux
    print("Init model")
    config = configs[name]

    # Use local_path if provided, otherwise download from HuggingFace
    if local_path is not None:
        ckpt_path = os.path.join(local_path, config.repo_flow)
...
def load_ae(name: str, device: str | torch.device = "cuda", local_path: str | None = None) -> AutoEncoder:
    config = configs[name]

    # Use local_path if provided, otherwise download from HuggingFace
    if local_path is not None:
        ckpt_path = os.path.join(local_path, config.repo_ae)
        if not os.path.exists(ckpt_path):
